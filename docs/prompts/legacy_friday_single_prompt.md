# Legacy Friday Single Prompt (Run + Verify)

Use this to run the Friday legacy status check and evaluate it in one pass.

```text
You are running and evaluating the Legacy Friday track.

Run commands in this order:
1) bash scripts/run_show_weekly.sh jack_mallers_show
2) bash scripts/report_mallers_friday_status.sh
