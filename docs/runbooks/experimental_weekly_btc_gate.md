...
```bash
python3 "$WORKSPACE/bitregime-core/scripts/gate_weekly_bundle.py" \
  --bundle-json "$WORKSPACE/sector-feeds/artifacts/private/weekly_bundles/weekly_critical_bundle.json" \
  --output-json "$WORKSPACE/bitregime-core/artifacts/gates/weekly_bundle_gate_status.json"
```
...
- `$WORKSPACE/bitregime-core/artifacts/gates/weekly_bundle_gate_status.json`
