# M-9 Intake Governance Agent (Independent)

## Objective

Promote intake-gate operations to milestone-agnostic governance while preserving M-5 policy-as-code, drift visibility, and rollback diagnostics.
...
3. revert latest M-9 governance commit(s) on branch if status/drift contracts regress.
4. restore last-known-good policy target and rerun daily gate.

## First Implementation Slice (M9-S1)

- add `milestones/m9_policy.json`
...
BITPOD_INTAKE_POLICY_JSON=milestones/m9_policy.json \
