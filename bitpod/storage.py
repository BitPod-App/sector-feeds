...
def _landing_page_html(*, permalink_id: str, base_url: str, public_status: dict[str, Any]) -> str:
    ...
    transcript_url = f"{landing_url}/transcript.md"
    ...
    return "\n".join(
        [
            ...
            "      button.ghost-button { cursor: pointer; font-family: inherit; }",
            "      button.ghost-button:disabled { opacity: 0.45; cursor: not-allowed; }",
            ...
            "      .tts-card { display: grid; gap: 12px; }",
            "      .tts-controls { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 4px; }",
            "      .tts-status { min-height: 1.5em; font-size: 14px; color: #b9bfce; }",
            ...
            f'          <a class="button" href="{html_escape(transcript_url)}">Open transcript</a>',
            "        </div>",
            "      </header>",
            ...
            *readability_rows,
            ...
            "          </article>",
            '          <article class="section-card">',
            '            <span class="eyebrow">Transcript audio</span>',
            '            <h3>Speak transcript for manual QA</h3>',
            '            <div class="tts-card">',
            '              <p class="muted">Use browser text-to-speech to listen for transcription glitches, punctuation errors, and awkward speaker breaks directly from the permalink transcript.</p>',
            '              <div class="tts-controls">',
            '                <button id="tts-start" class="button" type="button">Speak transcript</button>',
            '                <button id="tts-pause" class="ghost-button" type="button" disabled>Pause</button>',
            '                <button id="tts-resume" class="ghost-button" type="button" disabled>Resume</button>',
            '                <button id="tts-stop" class="ghost-button" type="button" disabled>Stop</button>',
            "              </div>",
            '              <p id="tts-status" class="tts-status">Speech preview ready when supported by this browser.</p>',
            "            </div>",
            "          </article>",
            '          <article class="section-card">',
            ...
            *[
            ...
            "    </main>",
            "    <script>",
            f"      const transcriptUrl = {json.dumps(transcript_url)};",
            "      const speechApiAvailable = typeof window !== 'undefined' && 'speechSynthesis' in window && typeof SpeechSynthesisUtterance !== 'undefined';",
            ...
            "        if (!startButton || !pauseButton || !resumeButton || !stopButton) return;",
            "        const disabled = !speechApiAvailable;",
            "        startButton.disabled = disabled || state === 'loading' || state === 'speaking' || state === 'paused';",
            "        pauseButton.disabled = disabled || state !== 'speaking';",
            "        resumeButton.disabled = disabled || state !== 'paused';",
            "        stopButton.disabled = disabled || (state !== 'speaking' && state !== 'paused');",
            "      }",
            ...
            "        transcriptText = raw.replace(/^---[\\s\\S]*?---\\s*/m, '').replace(/^#.*$/gm, '').replace(/\\[(.*?)\\]\\((.*?)\\)/g, '$1').replace(/[`*_>#-]/g, ' ').replace(/\\s+/g, ' ').trim();",
            ...
            "        if (!speechApiAvailable) return;",
            "        setTtsButtons('loading');",
            "        setTtsStatus('Loading transcript audio preview...');",
            "        try {",
            "          const text = await loadTranscriptText();",
            ...
            "            isPaused = false;",
            "            setTtsButtons('speaking');",
            "            setTtsStatus('Speaking transcript. Listen for transcription quality issues.');",
            "          };",
            ...
            "            isPaused = false;",
            "            setTtsButtons('idle');",
            "            setTtsStatus(`Speech failed: ${event.error || 'unknown error'}.`);",
            "          };",
            ...
            "          isPaused = false;",
            "          setTtsButtons('idle');",
            "          setTtsStatus(error instanceof Error ? error.message : 'Unable to start speech preview.');",
            "        }",
            "      }",
            "      if (!speechApiAvailable) {",
            "        setTtsButtons('unsupported');",
            "        setTtsStatus('Speech preview is not supported in this browser.');",
            "      } else {",
            "        setTtsButtons('idle');",
            "        startButton.addEventListener('click', () => { void startSpeech(); });",
            "        pauseButton.addEventListener('click', () => {",
            "          if (!activeUtterance || isPaused) return;",
            "          window.speechSynthesis.pause();",
            "          isPaused = true;",
            "          setTtsButtons('paused');",
            "          setTtsStatus('Speech paused.');",
            "        });",
