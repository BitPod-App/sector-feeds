...
def get_feed_urls(show: dict[str, Any]) -> list[str]:
    ...
    # Prefer RSS first for lower-cost direct audio ingestion when available.
...
def _choose_best_source(existing: Any, candidate: Any) -> Any:
    ...
    # same source rank -> keep newest publication timestamp
...
def _maybe_use_captions(
    *,
    ...
    caption_text = extract_youtube_captions(str(episode.source_url), caption_dir, min_words=min_caption_words)
    if not caption_text:
        return False
...
def _resolve_cached_media(show_key: str, existing: dict[str, Any] | None) -> Path | None:
    if not existing:
        return None
...
def _process_episode(
    *,
    ...
    media_path = _resolve_cached_media(show_key, existing)
